...
const FAQ = () => {
  ...
  return (
    <div id="faq" className="flex flex-col items-center justify-center px-4 sm:px-0">
      <div className="w-full max-w-[640px] py-10 sm:py-20 mt-4">
        <div className="flex flex-col items-center justify-center">
          <h1 className="nav-items text-primary">FAQ</h1>
          <div className="w-[400px] text-center mt-[10px]">
            <p>Do you have any questions?</p>
            <p>Here are a few answers for most common questions</p>
          </div>
        </div>
          ...
          {QNA.map((que, index) => (
            <div key={index}>
              <div className="border border-solid border-[#e6e6e6] bg-[#fcfcfc] p-[12px] rounded-[10px]">
                <div className="flex flex-row justify-between items-center cursor-pointer" onClick={() => toggleFAQ(index)}>
                ...
                >
                  {openIndex === index && (
                    <p className="text-neutral pl-4 pt-1">{que.answer}</p>
                  )}
                </div>
              </div>
            </div>
